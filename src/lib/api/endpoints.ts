...
import type { Seat, Booking, Invoice, Payment, UserSummary } from '@/lib/types'

/* AUTH */
export async function postLogin(payload: { email: string; password: string }): Promise<{ token: string; user: UserSummary }>{ 
  const { data } = await api.post('/auth/login', payload); 
  return data; 
}
export async function postLogout(){ await api.post('/auth/logout') }

/* PUBLIC */
export async function getSeats(params?: { status?: string }): Promise<Seat[]>{ 
  const { data } = await api.get('/seats', { params }); 
  return data; 
}
export async function getSeatById(id: number): Promise<Seat>{ 
