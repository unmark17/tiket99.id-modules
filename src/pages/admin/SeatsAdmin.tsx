    ...
    <div>
      ...
      <details className="mt-2">
        ...
        <form onSubmit={add} className="grid md:grid-cols-3 gap-2 mt-2">
          ...
          <input className="border rounded-xl px-3 py-2" value={form.cabin_class||''} onChange={e=>setForm({...form, cabin_class:e.target.value})} placeholder="Kabin" />
          <select className="border rounded-xl px-3 py-2" value={form.status as any} onChange={e=>setForm({...form, status: e.target.value as any})}>
            {['OPEN','LIMITED','CLOSED','HIDDEN'].map(x=> <option key={x} value={x}>{x}</option>)}
          </select>
